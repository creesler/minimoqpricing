...
router.post('/scrape-multi', async (req, res) => {
  ...
  try {
    ...
    for (const header of headers) {
      ...
      // ✅ Final condition check before DB write
      if (!$form?.length || !formFields.length || formFields.every(f => !f.options.length)) {
        console.log(`🚫 Skip save for "${formName}" — missing required form structure.`);
        continue;
      }
