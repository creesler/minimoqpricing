...
// ✅ Combination generator that excludes "Print Color" field
...
async function scrapeGroups(url) {
  ...
  const result = await page.evaluate(() => {
    ...
    containers.forEach(container => {
      ...
      if (labelText.includes('black')) {
        ...
        // default: shared dropdowns go in both
        blackFields.push(field);
        fullColorFields.push(field);
      }
    });
  ...
  console.log('\n🧾 Full Color fields from DB:');
  console.dir(fullColorSaved, { depth: null });

  console.log('\n🧾 Black fields from DB:');
  console.dir(blackSaved, { depth: null });

  const fullColorCombos = generateCombinations(fullColorSaved, 'Full Color');
  const blackCombos = generateCombinations(blackSaved, 'Black');
