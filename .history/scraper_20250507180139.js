...
function simplifyLabel(label) {
  ...
  return label
    ...
    .replace(/--.*$/, '') // remove trailing "--Full Color" or similar
    .trim();
}

// 🔧 Format keys to show simplified labels instead of select-n
...
// 🔧 Generate all combinations (excluding Print Color dropdown)
...
async function scrapeGroups(url) {
  ...
  // 🧩 Generate combinations from saved data
